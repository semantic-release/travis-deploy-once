...
module.exports = (versions, logger) => {
  ...
  let stable = versions.lastIndexOf('node') + 1;
  if (!stable && versions.indexOf('stable') !== -1) {
    // "stable" is only supported for backwards compatibility
    // see https://github.com/semantic-release/travis-deploy-once/issues/49
    stable = versions.lastIndexOf('stable') + 1;
  }
  if (stable) {
